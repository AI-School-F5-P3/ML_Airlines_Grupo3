...
# Función para solicitar datos del cliente y predecir
def collect_data_and_predict():
    ...
    print(client_data_scaled)

    # Verificar si hay valores NaN en client_data
    if client_data_scaled.isna().sum().sum() > 0:
        print("Error: Algunos datos contienen valores NaN. Por favor, verifique las entradas.")
        return

    # Realizar la predicción
    prediction = rf_model.predict(client_data_scaled)
    prediction_proba = rf_model.predict_proba(client_data_scaled)[:, 1]
